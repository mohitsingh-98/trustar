...
import { Image, ActivityIndicator, View, ScrollView, Text, TextInput, Keyboard, TouchableOpacity } from 'react-native';
import {KeyboardAwareScrollView} from 'react-native-keyboard-aware-scrollview'
import styles from './styles';
import { post } from '../../JS/service'
let otp = '';

export default class UserEmailVerify extends Component {
	...
	onVerifyOtp() {
		...
		post(data).then((res) => {
			if (res.ack == 1) {
				this.props.navigation.navigate('SignUpUserDetails', res)
			}
			else if (res.ack == 0) {
	...
	resendOtp() {
		...
		post(data).then((res) => {
			if (res.ack == 1) {
				clearInterval(this.interval);
				this.setState({ timer: 60, isResend: false });
